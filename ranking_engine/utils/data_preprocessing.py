...
def preprocess_supplier_data(transactions):
    ...
    for transaction in transactions:
        ...
        supplier_data[supplier_id]['total_items'] += transaction.quantity
        supplier_data[supplier_id]['defect_items'] += transaction.defect_count
        ...
        transaction_amount = transaction.quantity * unit_price
        supplier_data[supplier_id]['total_amount'] += transaction_amount
        
        # Calculate delivery performance
        if transaction.actual_delivery_date and transaction.expected_delivery_date:
            if transaction.actual_delivery_date <= transaction.expected_delivery_date:
                supplier_data[supplier_id]['on_time_orders'] += 1
            else:
                supplier_data[supplier_id]['late_orders'] += 1
                delay = (transaction.actual_delivery_date - transaction.expected_delivery_date).days
